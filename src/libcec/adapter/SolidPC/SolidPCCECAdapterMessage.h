...
namespace SolidPCCEC
{
  ...
  public:
    ...
    /*!
     * @brief HexToBin - convert a hex digit to its real value
     * @param ch ascii character represents hex digit
     *
     * HexToBin() converts one hex digit to its actual value or -1 in case of bad
     * input.
     */
    int8_t HexToBin(uint8_t ch) const;
    ...
    /*!
     * @brief Adds a hex nibble to this message.
     * @param byte The byte to add as hex, the low part is used.
     */
    void PushBackHexLo(uint8_t byte);

    /*!
     * @brief Adds a hex byte to this message.
     * @param byte The byte to add as hex.
     */
